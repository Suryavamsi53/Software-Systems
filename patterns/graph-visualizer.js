...
class Controller {
    ...
    initEventListeners() {
        // Toolbar
        const el_mode_move = document.getElementById('mode-move');
        if (el_mode_move) el_mode_move.onclick = () => this.setMode('move');
        const el_mode_node = document.getElementById('mode-node');
        if (el_mode_node) el_mode_node.onclick = () => this.setMode('node');
        const el_mode_edge = document.getElementById('mode-edge');
        if (el_mode_edge) el_mode_edge.onclick = () => this.setMode('edge');
        const el_mode_delete = document.getElementById('mode-delete');
        if (el_mode_delete) el_mode_delete.onclick = () => this.setMode('delete');
        
        const el_directed = document.getElementById('is-directed');
        if (el_directed) {
            el_directed.onchange = (e) => {
                this.graph.isDirected = e.target.checked;
                this.updateUI();
            };
        }
        
        const el_weighted = document.getElementById('is-weighted');
        if (el_weighted) {
            el_weighted.onchange = (e) => {
                this.graph.isWeighted = e.target.checked;
                this.updateUI();
            };
        }
        
        const el_grid = document.getElementById('is-grid-mode');
        if (el_grid) {
            el_grid.onchange = (e) => {
                ...
                const el_maze = document.getElementById('generate-maze');
                if (el_maze) el_maze.style.display = this.graph.isGrid ? 'block' : 'none';
            };
        }
        
        const el_clear = document.getElementById('clear-graph');
        if (el_clear) {
            el_clear.onclick = () => {
                this.graph.clear();
                this.updateUI();
        ...
        const el_maze_btn = document.getElementById('generate-maze');
        if (el_maze_btn) {
            el_maze_btn.onclick = () => {
                this.generateMaze();
            };
        ...
        if (el_btn_start) el_btn_start.onclick = () => this.startAlgo();
        const el_btn_step = document.getElementById('btn-step');
        if (el_btn_step) el_btn_step.onclick = () => this.stepAlgo();
        const el_btn_pause = document.getElementById('btn-pause');
        if (el_btn_pause) el_btn_pause.onclick = () => this.pauseAlgo();
        const el_reset_algo = document.getElementById('reset-algo');
        if (el_reset_algo) el_reset_algo.onclick = () => this.resetAlgo();
        
        const el_speed = document.getElementById('speed-range');
        if (el_speed) {
            el_speed.oninput = (e) => {
        ...
        // Algorithm Select Change (if exists)
        const el_algo_select = document.getElementById('algo-select');
        if (el_algo_select) {
            el_algo_select.onchange = (e) => {
    ...
    zoom(direction) {
        const container = document.querySelector('.viz-viewport');
        if (!container) return;
        
        const rect = container.getBoundingClientRect();
        const rawPos = { x: rect.width / 2, y: rect.height / 2 };
    ...
    startAlgo() {
        ...
        if (!this.algoIterator) {
            const type = window.currentAlgorithm || 'bfs';
    ...
    highlightCodeLine(lines) {
        if (window.highlightLine && lines && lines[this.currentLang]) {
            window.highlightLine(lines[this.currentLang]);
        }
    }
    ...
    loadExample(type) {
        ...
        if (type === 'tree') {
            ...
            for(let i=0; i<numNodes; i++) {
                ...
                this.graph.addNode(cx + 150 * Math.cos(angle), cy + 150 * Math.sin(angle));
            }
            // Add edges
            for(let i=0; i<numNodes; i++) {
                this.graph.addEdge(i, (i+1)%numNodes);
                this.graph.addEdge(i, (i+2)%numNodes);
            }
            this.graph.isDirected = false;
        } else if (type === 'bipartite') {
            // Group 1
            this.graph.addNode(cx - 100, cy - 100);
            this.graph.addNode(cx - 100, cy);
            this.graph.addNode(cx - 100, cy + 100);
            // Group 2
            this.graph.addNode(cx + 100, cy - 50);
            this.graph.addNode(cx + 100, cy + 50);
            ...
            // Component 1
            this.graph.addNode(cx - 150, cy - 50);
            this.graph.addNode(cx - 50, cy - 50);
            this.graph.addNode(cx - 100, cy + 50);
            ...
            // Component 2
            this.graph.addNode(cx + 100, cy - 50);
            this.graph.addNode(cx + 200, cy + 50);
            this.graph.addEdge(3, 4);
            // Component 3
            this.graph.addNode(cx + 50, cy + 100);
            this.graph.isDirected = false;
        } else if (type === 'grid') {
            // Create a 10x10 grid
            ...
            const gridCheck = document.getElementById('is-grid-mode');
            if(gridCheck) gridCheck.checked = true;
            const mazeGen = document.getElementById('generate-maze');
            if(mazeGen) mazeGen.style.display = 'block';
        }
        
        const topSelect = document.getElementById('topology-select');
        if (topSelect) topSelect.value = type;
