...
// Ensure Google Roadmap link exists in the left sidebar nav
function ensureRoadmapNav(){
  try{
    const nav = document.querySelector('.sidebar-nav');
    if(!nav) return;
    // avoid duplicate
    if(nav.querySelector('a[href="google-roadmap/index.html"]')) return;
    // find visualizer link to insert after
    const visualizerLink = nav.querySelector('a[href="visualizer-dashboard.html"]');
    const a = document.createElement('a');
    a.href = 'google-roadmap/index.html';
    a.className = 'nav-item';
    a.textContent = '🗺️ Google Roadmap';
    if(visualizerLink && visualizerLink.parentNode){
      visualizerLink.parentNode.insertBefore(a, visualizerLink.nextSibling);
    } else {
      nav.insertBefore(a, nav.firstChild);
    }
  }catch(e){console.warn('ensureRoadmapNav failed',e)}
}
